...
class object
{
	...
	public:
		...
		friend void F_Count_Blocks(char water[][MAPY], char map[][MAPY], int x, int y); 
};
class Snake
{
	int headx, heady;
...
char Snake::AI(char water[][MAPY], class object MAP)
{
	...
	F_Count_Blocks(Count_Blocks, MAP.map, headx, heady);
	for(a = 0; a < MAPX; a++)
	{
		for(b = 0; b < MAPY; b++)
		{
...
void F_Count_Blocks(char water[][MAPY], char map[][MAPY], int x, int y)
{
	int a, b;
	int stx[2][MAPX * MAPY], sty[2][MAPX * MAPY], top, top2, distance;
...
int main(void)
{
	...
	cout<<"滿。│\n";
	cout<<"└┬────────────────────────┬─┘\n";
	cout<<"　│http://www.cs.nctu.edu.tw/~cychih/Snake_IV_4.exe│\n";
	cout<<"　└────────────────────────┘\n";
	cout<<"\n\n\n";
	cout<<"請按任意鍵結束遊戲 >>\n\n";
	pause();
