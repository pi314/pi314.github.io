...
char Snake::AI(char water[][MAPY], class object MAP)
{
	...
	/*數格子*/ 
		...
		max = 0;
		if(headx - 1 >= 0)
		{// && MAP.map[headx - 1][heady] == GROUND
			FindPath(Count_Blocks, MAP.map, headx - 1, heady, headx, heady);
			for(a = 0; a < MAPX; a++)
			{
				for(b = 0; b < MAPY; b++)
				{
		...
		if(heady - 1 >= 0)
		{
			FindPath(Count_Blocks, MAP.map, headx, heady - 1, headx, heady);
			for(a = 0; a < MAPX; a++)
			{
				for(b = 0; b < MAPY; b++)
				{
			...
			if(count_blocks[1] == count_blocks[max])
			{
			...
			else if(count_blocks[1] > count_blocks[max])
			{
				max = 1;
			}
		}
		if(heady + 1 < MAPY)
		{
			...
			if(count_blocks[2] == count_blocks[max])
			{
				if(water[headx][heady + 1] < water[headx][heady])
				{
					max = 2;
			...
			else if(count_blocks[2] > count_blocks[max])
			{
				max = 2;
			}
		}
		if(headx + 1 < MAPX)
		{
			...
			if(count_blocks[3] == count_blocks[max])
			{
			...
			else if(count_blocks[3] > count_blocks[max])
			{
				max = 3;
			}
		}
	/*數格子結束*/
		...
		{
			change = 0;
			if(water[headx + dirx][heady + diry] < water[headx][heady])
			{
				temp = NO_REACTION;
			}
